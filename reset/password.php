<?php 
    ...
    if ($_SERVER["REQUEST_METHOD"] == "GET") {
        ...
        if($idno != null || $idno != ""){
            $query = "SELECT idno,firstname,lastname,role,profilePath FROM users WHERE idno = '$idno' AND role = 'student' LIMIT 1";
...
<body class="dark bg-gray-900">
    ...
    <div class="px-4 mb-5 mt-5">
        <h2 class="mb-4 text-center text-2xl m-auto font-extrabold leading-none tracking-tight text-gray-900 md:text-4xl dark:text-white">Reset Password</h2>
        ...
        <div class="mt-5 w-full mx-auto p-4 max-w-md bg-white border border-gray-200 rounded-lg shadow dark:bg-gray-800 dark:border-gray-700">
            <?php if(isset($student_detail['idno'])):?>  
                ...
                <div class="flex flex-col items-center pb-5">
                    ...
                    <form class="flex justify-between items-center gap-3 mt-4 md:mt-6 w-full" action="resetpassword.php" method="post">
                        <input type="password" name="password" id="small-input" class="block w-full p-2 text-gray-900 border border-gray-300 rounded-lg bg-gray-50 text-xs focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="New Password" required>
                        <input name="idno" value="<?= $student_detail['idno']?>" hidden/>
                        <button type="submit" class="inline-flex items-center whitespace-no-wrap px-4 py-2 text-sm font-medium text-center text-white bg-blue-700 rounded-lg hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800">Change</button>
                    </form>
                </div>
                <div class="text-sm font-medium text-gray-700 dark:text-gray-500 text-center pb-5">
                    <p class="my-1">Set a new password or Default Reset</p>
                    <p>Default Reset will reset password to 'password123'</p>
                </div>
            <?php else:?>  
